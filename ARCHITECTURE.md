...
The neutral brand definition owns named colors, optional free-form role aliases, typography, and asset paths. Adapters own target roles and output. `id` maintains the Nuxt UI adapter and a small CSS-variable reference adapter; user adapters remain ordinary TypeScript modules. There is no adapter registry or discovery runtime.
