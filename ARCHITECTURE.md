...
Original and draft render in separate frames. Host routes use the same origin; independent development apps opt into `@happydesigns/id/preview`. Both origins are explicitly configured, and external connections additionally bind messages to a session. Messages require the configured origin and window identity. The same preview plugin serves both integrations; the development module adds no Studio pages, writer or production plugin. The editor temporarily previews draft UI defaults in the shell and restores the host configuration when leaving Studio. Local draft storage is scoped to the host brand. Browser-local project recovery preserves drafts during switches; when storage fails, replacing unsaved work requires a user choice. Export requires no server storage or repository credentials.
