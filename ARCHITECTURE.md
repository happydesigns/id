...
`nuxt.config.ts` is intentionally a repository-development config. It imports the public layer config and can add `@nuxt/eslint` and other local tooling because it is not the `./nuxt` package export. Consumers extending `@happydesigns/id/nuxt` inherit the public layer only: Nuxt UI, `Id` components, identity CSS, runtime MDC highlighting for install code groups, and the current layer compatibility defaults. The Docus generated-CSS compatibility module only marks Docus's generated `docus.css` template as disk-written when Docus has registered it, so non-Docus consumers do not get server middleware or extra CSS behavior.
