...
- Use runtime theme packs only for values that can change without recompiling UI.
...
| --- | --- | --- |
| `src/` | Brand-guide types, validation, CSS variable generation, app-config helpers, theme-pack utilities. | Vue component state, routing, assets, app-specific copy. |
...
Runtime theme packs are for switching visual roles without rebuild overhead. They can own CSS variables, Nuxt UI semantic color mappings, typography variables, and component default variants that use stable compiled classes.
