...
<template>
  ...
  <div
    v-if="docked && open"
    class="studio-editor-docked"
    @keydown.esc="if (!$event.defaultPrevented) { open = false; emit('restoreFocus') }"
  >
    <Content />
  </div>
  <UPopover
    ...
    :portal="false"
    :content="content"
    :ui="{ content: 'studio-editor-overlay flex flex-col w-80 max-w-[calc(100vw-2rem)] max-h-[min(80dvh,var(--reka-popover-content-available-height))] overflow-hidden' }"
  >
    <template #anchor>
      <span
        class="studio-editor-anchor"
        aria-hidden="true"
      />
    </template>
    <template #content>
