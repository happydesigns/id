...
import { diffStudioDocuments, parseStudioDocument } from '@happydesigns/id/studio/core'
...
export async function readStudioSource(path: string) {
  ...
  if (!stat.isFile() || stat.isSymbolicLink() || stat.size > 8_000_000) throw new Error('Choose a regular brand JSON source smaller than 8 MB.')
...
export async function writeStudioSource(path: string, revision: string, input: unknown) {
  ...
  if (Buffer.byteLength(content, 'utf8') > 8_000_000) throw new Error('The brand source must be smaller than 8 MB.')
