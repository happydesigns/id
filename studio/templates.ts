...
export function studioTemplates(input?: unknown): StudioTemplate[] {
  ...
  for (const [id, value] of Object.entries(input)) {
    ...
    if (isStudioRoute(item.route) && isStudioRoute(item.routePrefix) && withinStudioRoute(item.route, item.routePrefix)) {
      result.push({ id, label: item.label, description: typeof item.description === 'string' ? item.description : '', route: item.route, routePrefix: item.routePrefix, owner: typeof item.owner === 'string' ? item.owner : undefined, pages: [{ id: 'home', label: 'Home' }] })
      continue
    }
