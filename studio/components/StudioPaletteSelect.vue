  ...
  <UPopover v-model:open="open" :content="{ align: 'start' }">
    ...
    <template #content>
      <div class="grid w-80 max-w-[calc(100vw-2rem)] grid-cols-3 gap-1 p-2 max-h-[min(65dvh,28rem)] overflow-y-auto" :aria-label="`${label} palettes`">
        ...
        <p class="col-span-3 px-2 pt-2 text-xs text-muted">{{ group.label }}</p>
        <UButton v-for="name in group.items" :key="name" color="neutral" variant="ghost" size="sm" :aria-label="name" :aria-pressed="model === name" :active="model === name" active-variant="soft" class="min-w-0" @click="select(name)">
        ...
        <UButton color="neutral" variant="link" size="xs" class="col-span-3" @click="all = !all">{{ all ? 'Show suggested palettes' : 'Show all palettes' }}</UButton>
      </div>
    </template>
  </UPopover>
</template>
