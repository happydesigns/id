...
const items = computed<StudioTemplate[]>(() => [{
  id: 'components',
  label: 'Components',
  description: 'Forms, navigation and interactive UI components.',
  thumbnail: '/id-studio/templates/components.png',
  pages: [],
}, ...props.templates])
...
<template>
  <div class="template-switcher">
    <UPopover v-model:open="open" :content="{ align: 'end', sideOffset: 8 }">
      <UButton color="neutral" variant="ghost" trailing-icon="i-lucide-chevron-down" aria-label="Templates" class="templates-trigger" :ui="{ trailingIcon: 'ms-auto shrink-0' }"><span class="truncate">{{ selected?.label || 'Components' }}</span></UButton>
      <template #content>
...
@media (max-width: 700px) { .template-gallery-wide { grid-template-columns: repeat(2, minmax(0, 1fr)); } }
</style>
