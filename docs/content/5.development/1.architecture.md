...
Neutral brand definitions own colors, open typography roles and structured assets. Adapters own target-system output and remain ordinary TypeScript modules; there is no adapter registry or discovery runtime. The existing identity runtime consumes `BrandTheme`; optional guide data is not required by adapters or normal applications. See the [API reference](/reference/api) for exact runtime and guide config types.
