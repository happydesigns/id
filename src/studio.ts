...
export function createStudioProject(input: StudioDocument, options: { bundledPackage?: boolean, legacyRuntime?: boolean, guide?: boolean } = {}): Record<string, string> {
  const doc = parseStudioDocument(input)
  const json = (value: unknown) => `${JSON.stringify(value, null, 2)}\n`
  const files: Record<string, string> = {
    'brand.studio.json': json(doc),
    'package.json': json({ name: doc.brand.packageName || doc.brand.name, private: true, type: 'module', scripts: { dev: 'nuxt dev playground', build: 'nuxt build playground' }, dependencies: { '@happydesigns/id': options.bundledPackage ? 'file:./vendor/id.tgz' : '^0.2.0', '@nuxt/ui': '4.10.0', nuxt: '4.5.2', tailwindcss: '4.3.3', '@iconify-json/lucide': '^1.2.123' } }),
  ...
  if (options.guide) {
    delete project['playground/app/app.vue']
    return project
  }
  const manifest = JSON.parse(project['package.json']!)
  delete manifest.devDependencies.docus
  delete manifest.devDependencies['@takumi-rs/core']
  project['package.json'] = json(manifest)
  delete project['playground/content/docs/1.introduction.md']
  delete project['playground/content/docs/2.brand-reference.md']
  delete project['playground/app/app.config.ts']
  project['playground/nuxt.config.ts'] = `import { fileURLToPath } from 'node:url'\nimport document from '../brand.studio.json'\nimport { generateBrand } from '../scripts/generate-brand.mjs'\nexport default defineNuxtConfig({ extends: ['..', '@happydesigns/id/studio'], compatibilityDate: '2026-08-01', css: ['~/app.css'], icon: { serverBundle: { collections: ['lucide', 'vscode-icons'] } }, runtimeConfig: { idStudioSource: fileURLToPath(new URL('../brand.studio.json', import.meta.url)) }, hooks: { 'builder:watch': (_event, path) => { if (path.endsWith('brand.studio.json')) generateBrand() } }, appConfig: { idStudio: { document, sourcePath: 'brand.studio.json'${options.bundledPackage ? ", packageAsset: '/studio-packages/id.tgz'" : ''} } } })\n`
  project['playground/app/app.css'] = '@import "tailwindcss";\n@import "@nuxt/ui";\n@import "../../app/assets/css/brand.css";\n'
  project['README.md'] = `# ${doc.theme.label}\n\nA native Nuxt UI brand layer. Consumers extend ${doc.brand.packageName || doc.brand.name} and import its styles.css after Tailwind and Nuxt UI in their own CSS entry. Consumers need neither ID nor Docus.\n\nRun pnpm install and pnpm dev to open Studio with Components and Landing. Add capability templates explicitly in appConfig.idStudio.templates. The editable source is brand.studio.json; Apply updates it locally after a revision check. Run pnpm generate:brand after external edits. Generated app files are derived output.\n\nDocus documentation is optional: select Include Docus guide when exporting a new project, or add docus and @happydesigns/id/guide to a separate documentation host. Existing guides remain supported.\n\n${options.bundledPackage ? 'The id development package is bundled in vendor/id.tgz.' : 'Until id is published, use a reviewed local id package as the development dependency.'}\n\nFonts must be available to the consuming build. Custom domain components and capabilities are not included.\n`
  return project
}
