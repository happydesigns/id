...
The header has one project menu, one preview selector and a quiet Changes/Download action. The inspector uses Nuxt UI Accordion, FormField, Select, ColorPicker and FileUpload controls. Place persistence status once below it. Keep the editable draft out of the shell app config. Apply advances the baseline only after a successful source revision check. Docus owns its complete header, search, navigation and page layout; host branding uses its documented config and small slots.
