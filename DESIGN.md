...
The Studio is a viewport-sized workspace: its header and dock remain visible, and each isolated preview owns its document scroll. Do not introduce a minimum preview height that makes the host page scroll. Settings have their own bounded inspector, which can be closed to restore the full canvas; on narrow screens it replaces the visible canvas until the author returns to Preview. Component examples use a responsive, content-sized gallery without a second scrolling wrapper. Keep examples interactive and use Nuxt UI components with the current brand tokens.
...
The header has a visible main menu on the left, a centered group with brand picker followed by template selection, and a quiet Changes/Download action on the right. On narrow screens the menu keeps its recognizable menu icon and the two selectors share the remaining width. The picker contains configured identities and browser drafts only. Nuxt UI is a protected starting point; customization creates an independent brand. The inspector uses Nuxt UI Accordion, FormField, Select, ColorPicker and FileUpload controls. Place persistence status once below it. Keep the selected brand and editable draft out of the shell app config. Apply advances the baseline only after a successful source revision check. Docus owns its complete header, search, navigation and page layout; host branding uses its documented config and small slots.
