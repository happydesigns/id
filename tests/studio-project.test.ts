...
describe('native runtime and real previews', () => {
  it('exports runtime files without an id runtime import and with optional authoring dependencies', () => {
    ...
    expect(files['app/app.config.ts']).not.toContain('import')
    expect(files['app/components/BrandLogo.vue']).toContain('UColorModeImage')
    expect(files['playground/app/app.vue']).toBeUndefined()
    expect(files['playground/nuxt.config.ts']).toContain("image: { provider: 'none' }")
    expect(files['playground/app/app.config.ts']).toContain('github: false')
