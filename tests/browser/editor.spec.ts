...
test('standard palettes can be overridden and reset in the editor', async ({ page }) => {
  await page.goto('/studio?editor=colors&mode=light&docked=true')
  ...
  await page.getByRole('button', { name: 'New palette', exact: true }).click()
  const modal = page.getByRole('dialog').filter({ has: page.getByRole('heading', { name: 'New palette', exact: true }) })
  await modal.getByRole('textbox', { name: /^Palette name/ }).fill('blue')
  await modal.getByRole('button', { name: 'Create palette', exact: true }).click()
  await expect(modal).toBeHidden()
  await expect(page.getByText('Overrides the standard palette.', { exact: false })).toBeVisible()
  await expect.poll(blue).toBe('#2563eb')
  await page.getByRole('button', { name: 'Reset palette blue', exact: true }).click()
  await expect.poll(blue).toBe(original)
  await expect(page.getByRole('button', { name: 'Actions for palette blue', exact: true })).toHaveCount(0)
})
