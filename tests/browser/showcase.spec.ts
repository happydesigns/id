...
for (const width of [1440, 390]) test('brand showcase separates browsing from authoring at ' + width, async ({ page }) => {
  ...
  await expect(email).toHaveValue('preserved@example.com')
  await expand.click()
  await showcase.getByRole('button', { name: 'Hide preview controls' }).click()
  await expect(showcase.getByRole('button', { name: 'Show preview controls' })).toBeFocused()
  await showcase.getByRole('button', { name: 'Show preview controls' }).click()
