...
describe('optional studio templates', () => {
  it('keeps the base studio usable without capability packages', () => {
    expect(studioTemplates().map(item => item.id)).toEqual(['landing', 'docs'])
  })
  ...
  it('rejects reserved identifiers, arbitrary element names and ambiguous pages', () => {
    ...
    expect(studioTemplates({ components: entry, bad: { ...entry, component: 'iframe' }, duplicate: { ...entry, pages: [...entry.pages, ...entry.pages] }, path: { ...entry, pages: [{ id: '../route', label: 'Route' }] } })).toHaveLength(2)
  })
})