...
describe('starter templates', () => {
  it('keeps the package nuxt export on the consumer-safe layer config', () => {
    ...
    expect(packageJson.files.some(file => file.startsWith('templates/'))).toBe(false)
    expect(packageJson.files).not.toContain('templates/brand-layer/docs')
  ...
  it('keeps the brand-layer template shaped like an external brand repo', () => {
    ...
    expect(readTemplateFile('brand-layer', 'app/app.config.ts')).toContain('import brand from \'./brand.config\'')
    expect(readTemplateFile('brand-layer', 'app/brand.config.ts')).toContain('Generated from brand.studio.json')
