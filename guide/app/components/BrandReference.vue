...
<template>
  <div
    ...
    data-brand-reference
  >
    <template v-if="show('colors')">
      <section
        aria-label="Brand palettes"
        class="space-y-6"
      >
        ...
        <p
          v-if="!reference.palettes.length"
          class="text-sm text-muted"
        >
          No custom palettes. This brand uses Nuxt UI colors.
        </p>
        <div
          v-for="palette in reference.palettes"
          :key="palette.name"
          class="space-y-3"
        >
          ...
          <ul class="grid grid-cols-2 gap-3 sm:grid-cols-3 xl:grid-cols-4">
            ...
            >
              <div
                class="h-12 border-b border-default"
      ...
      >
        ...
        <div class="overflow-x-auto rounded-lg border border-default">
          <table class="w-full text-left text-sm">
            <caption class="sr-only">
              Configured Nuxt UI color roles
            </caption>
        ...
        <p class="text-xs text-muted">
          Unspecified roles follow Nuxt UI defaults.
        </p>
      </section>
      <section
      ...
      >
        ...
        <p class="text-sm text-muted">
          Explicit brand values. Dark mode inherits light values unless overridden; other values follow Nuxt UI defaults.
        </p>
        <div class="overflow-x-auto rounded-lg border border-default">
          <table class="w-full table-fixed text-left text-xs">
            <caption class="sr-only">
              Brand CSS variables by color mode
            </caption>
            <thead class="bg-muted">
              <tr>
                <th
            ...
            <tbody>
              <tr
                v-for="variable in reference.variables"
                :key="variable.name"
                class="border-t border-default"
              >
                <th
                  class="break-all p-3 font-mono font-normal"
                  scope="row"
                >
                  {{ variable.name }}
                </th><td class="break-words p-3 font-mono">
                  {{ variable.light ?? 'Nuxt UI default' }}
                </td><td class="break-words p-3 font-mono">
                  {{ variable.dark ?? 'Nuxt UI default' }}
                </td>
              </tr>
            </tbody>
          </table>
      ...
      >
        <h2 class="text-xl font-semibold text-highlighted">
          Text contrast<span v-if="mounted"> · {{ colorMode.value }}</span>
        </h2>
        <p class="text-sm text-muted">
          Selected opaque text colors against the current default surface. Normal text needs at least 4.5:1. These checks do not assess every component or interaction state.
        </p>
        <ul class="grid gap-3 sm:grid-cols-2">
          <li
            v-for="pair in contrastPairs"
            :key="pair"
            class="space-y-2 rounded-lg border border-default p-3 text-sm"
          >
            <span
              :data-contrast-pair="pair"
              :style="{ color: `var(${pair})`, backgroundColor: 'var(--ui-bg)' }"
              class="block rounded p-2"
            >Sample text</span>
    ...
    >
      ...
      <p
        v-if="!reference.typography.length"
        class="text-sm text-muted"
      >
        Typography follows Nuxt UI defaults.
      </p>
      <article
        v-for="font in reference.typography"
        :key="font.role"
      ...
      >
        ...
        <p
          ...
          :style="{ fontFamily: `var(--font-${font.role}, ${font.family})` }"
        >
        ...
        <dl class="space-y-2 text-xs">
          <div>
            <dt class="font-medium">
              Light
            </dt><dd class="break-words font-mono text-muted">
              {{ font.light }}
            </dd>
          </div><div>
            <dt class="font-medium">
    ...
    >
      ...
      <p
        v-if="!reference.icons.length"
        class="text-sm text-muted"
      >
        Icons follow Nuxt UI defaults.
      </p>
      <ul class="grid gap-3 sm:grid-cols-2">
        <li
          v-for="item in reference.icons"
          :key="item.role"
          class="flex min-w-0 gap-3 rounded-lg border border-default p-3"
        >
          <UIcon
            :name="item.icon"
          ...
          <div class="min-w-0 text-sm">
            <span class="font-medium">{{ item.role }}</span><code class="block break-all text-xs text-muted">{{ item.icon }}</code>
          </div>
        </li>
      </ul>
    </section>
    <section
      v-if="show('styles')"
      aria-label="Configured styles"
      class="space-y-4"
    >
      <h2 class="text-xl font-semibold text-highlighted">
        Styles
      </h2>
      <dl class="grid grid-cols-2 gap-3 rounded-lg border border-default p-4 text-sm">
        <div>
          <dt class="font-medium">
            Corner radius · Light
          </dt><dd>{{ reference.radius.light ?? 'Nuxt UI default' }}</dd>
        </div><div>
          <dt class="font-medium">
            Corner radius · Dark
          </dt><dd>{{ reference.radius.dark ?? 'Nuxt UI default' }}</dd>
        </div>
      </dl>
      <p class="text-sm text-muted">
        Only explicit component overrides are listed. Other settings use Nuxt UI defaults.
      </p>
      <details
        v-for="component in reference.components"
        :key="component.name"
    ...
    >
      ...
      <ul class="grid gap-4 sm:grid-cols-2">
        <li
          v-for="asset in reference.assets"
          :key="asset.key"
          class="min-w-0 overflow-hidden rounded-lg border border-default"
        >
          <div
            v-if="isGuideAssetPath(asset.src) && imageAsset(asset.src)"
            class="flex h-32 items-center justify-center p-5"
            :style="{ background: asset.media === 'dark' ? '#18181b' : '#fafafa' }"
          >
            <img
              :src="asset.src"
          ...
          <div class="space-y-2 p-4 text-sm">
            <h3 class="font-medium">
              ...
              {{ asset.role }} · {{ asset.media ?? 'any' }}
            </p><a
              v-if="isGuideAssetPath(asset.src)"
              :href="asset.src"
              download
              class="break-all text-primary underline underline-offset-4"
            >Download {{ asset.name }}</a><p
              v-else
  ...
  <p
    ...
    class="text-sm text-muted"
  >
    Connect a brand source to display its reference.
  </p>
</template>
