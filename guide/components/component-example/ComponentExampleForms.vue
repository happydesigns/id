...
<template>
  ...
  >
    <div class="grid gap-4 md:grid-cols-2">
      <UFormField :label="t('forms.projectName')" :help="t('forms.projectNameHelp')">
        <UInput v-model="projectName" />
      </UFormField>
      <UFormField :label="t('forms.reviewStatus')" :help="t('forms.reviewStatusHelp')">
        <USelect v-model="status" :items="statusItems" />
      </UFormField>
    </div>
    <UFormField :label="t('forms.implementationNote')" :help="t('forms.noteHelp')">
      <UTextarea v-model="note" :rows="3" spellcheck="false" />
    </UFormField>
    <UFormField :label="t('forms.package')" :help="t('forms.packageHelp')">
      <UFieldGroup class="max-w-md">
        <UInput :model-value="props.context.packageName" readonly />
        <UButton color="neutral" variant="outline" icon="i-lucide-copy" :aria-label="t('forms.copyPackage')" />
      </UFieldGroup>
    </UFormField>
  ...
  <div v-else-if="props.name === 'text-entry-pattern'" class="grid gap-4 md:grid-cols-2">
    <UFormField :label="t('forms.shortText')" :help="t('forms.shortTextHelp')">
      <UInput v-model="projectName" />
    </UFormField>
    <UFormField :label="t('forms.longerNote')" :help="t('forms.longNoteHelp')">
      <UTextarea v-model="note" :rows="4" spellcheck="false" />
    </UFormField>
  </div>

  <div v-else-if="props.name === 'selection-pattern'" class="grid gap-4 md:grid-cols-2">
    <UFormField :label="t('forms.knownStatus')" :help="t('forms.knownStatusHelp')">
      <USelect v-model="status" :items="statusItems" />
    </UFormField>
    <UFormField :label="t('forms.searchableReviewer')" :help="t('forms.reviewerHelp')">
      <USelectMenu v-model="reviewer" :items="reviewerItems" value-key="value" />
    </UFormField>
    <UFormField :label="t('forms.exactPackage')" :help="t('forms.packageSuggestionsHelp')">
      <UInputMenu v-model="packageName" :items="packageItems" />
    </UFormField>
    <UFormField :label="t('forms.visibleSection')" :help="t('forms.sectionHelp')">
  ...
  <div v-else-if="props.name === 'choice-pattern'" class="grid gap-5 md:grid-cols-2">
    <div class="space-y-3">
      <UCheckbox v-model="includeNotes" :label="t('forms.includeNotes')" />
      <USwitch v-model="enabled" :label="t('forms.enabled')" />
    </div>
  ...
  <div v-else-if="props.name === 'bounded-pattern'" class="grid gap-5 md:grid-cols-2">
    <UFormField :label="t('forms.patternCount')" :help="t('forms.countHelp')">
      <UInputNumber v-model="budget" :min="1" :max="12" class="max-w-xs" />
    </UFormField>
    <UFormField :label="t('forms.reviewWindow')" :help="t('forms.reviewWindowHelp')">
      <div class="grid gap-2 sm:grid-cols-2">
        <UInputDate icon="i-lucide-calendar" class="w-full" />
        <UInputTime class="w-full" />
      </div>
    </UFormField>
    <UFormField :label="t('forms.intensity')" :help="t('forms.intensityHelp')">
      <USlider v-model="progress" />
    </UFormField>
    <UFormField :label="t('forms.accessCode')" :help="t('forms.accessCodeHelp')">
      <UPinInput v-model="pin" />
    </UFormField>
  </div>

  <div v-else-if="props.name === 'advanced-inputs-pattern'" class="grid gap-5 md:grid-cols-2">
    <UFormField :label="t('forms.labels')" :help="t('forms.tagsHelp')">
      <UInputTags v-model="tags" />
    </UFormField>
    <UFormField :label="t('forms.assetUpload')" :help="t('forms.uploadHelp')">
      <UFileUpload :label="t('forms.uploadLabel')" :description="t('forms.uploadFormats')" icon="i-lucide-upload" />
    </UFormField>
    <UFormField :label="t('forms.accent')" :help="t('forms.accentHelp')">
      <UColorPicker v-model="accent" />
    </UFormField>
    <UFormField :label="t('forms.publishingDate')" :help="t('forms.dateHelp')">
      <UCalendar />
    </UFormField>
  </div>

  <UForm v-else-if="props.name === 'form'" :state="{ projectName, status }" class="grid gap-4 sm:grid-cols-2">
    <UFormField :label="t('forms.projectName')" :help="t('forms.formNameHelp')">
      <UInput v-model="projectName" />
    </UFormField>
    <UFormField :label="t('forms.reviewStatus')" :help="t('forms.formStatusHelp')">
      <USelect v-model="status" :items="statusItems" />
    </UFormField>
  ...
  <UFormField v-else-if="props.name === 'form-field'" :label="t('forms.projectName')" :help="t('forms.fieldHelp')">
    <UInput v-model="projectName" />
  </UFormField>
  ...
  <UCheckbox v-else-if="props.name === 'checkbox'" v-model="includeNotes" :label="t('forms.includeNotes')" />
  ...
  <USwitch v-else-if="props.name === 'switch'" v-model="enabled" :label="t('forms.enabled')" />
  <USlider v-else-if="props.name === 'slider'" v-model="progress" />
  <UPinInput v-else-if="props.name === 'pin-input'" v-model="pin" />
  <UFileUpload v-else-if="props.name === 'file-upload'" :label="t('forms.uploadLabel')" :description="t('forms.uploadFormats')" icon="i-lucide-upload" />
